# context-engineering sub-skills 설계

**작성일**: 2026-04-25  
**목적**: context-engineering 워크플로우의 특정 Phase를 직접 호출할 수 있는 sub-skills 설계
...
이를 위해 Phase 단위로 직접 진입할 수 있는 sub-skills를 `context-engineering` 플러그인에 추가한다.
...
|------|-----------|---------------|
| `/context-engineering:gather` | Step 0 + Phase 1 (KB) + Phase 2 (CLAUDE.md) | 없어도 시작 가능 |
| `/context-engineering:spec`   | Phase 3 — PRD + SPEC | KB 있으면 로드 |
| `/context-engineering:impl`   | Phase 4 — 구현 | SPEC 있으면 로드 |
| `/context-engineering:valid`  | Readiness Gate 체크 | PRD·SPEC·KB·CLAUDE.md 확인 |
...
context-engineering/
├── .claude-plugin/
│   └── plugin.json          (name: "context-engineering")
├── skills/
│   ├── gather/SKILL.md
│   ├── spec/SKILL.md
│   ├── impl/SKILL.md
...
기존 `context-engineering` 레포 내에 sub-skills를 추가한다.  
설치 후 `/context-engineering:gather` 형태로 호출된다.
...
```
1. 인자 있음:  /context-engineering:spec @/path/to/project  →  해당 경로를 CODE_PATH로 사용
...
**종료:** "gather 완료 — /context-engineering:spec 으로 PRD·SPEC을 작성하거나 /context-engineering:valid 로 체크할 수 있습니다."
...
**진행:**
```
Phase 3-1 (PRD) → Phase 3-2 (SPEC) → /context-engineering:valid 제안
```
...
- sub-skills는 독립 SKILL.md로 작성 (기존 context-engineering SKILL.md에 의존하지 않음)
